...
def _connector() -> tuple[SnykToolSet, MockTransport]:
    transport = MockTransport()
    connector = SnykToolSet(api_token="snyk_secret", transport=transport)
...
def test_snyk_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "SNYK_list_organizations": "organizations",
        "SNYK_list_projects": "projects",
        "SNYK_list_issues": "vulns",
    }
...
def test_snyk_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_organizations)
