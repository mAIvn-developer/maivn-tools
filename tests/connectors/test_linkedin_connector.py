...
def _connector() -> tuple[LinkedInToolSet, MockTransport]:
    transport = MockTransport()
    connector = LinkedInToolSet(access_token="li_secret", transport=transport)
...
def test_linkedin_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "LINKEDIN_list_posts_for_author": "posts",
        "LINKEDIN_list_comments": "comments",
    }
...
def test_linkedin_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_posts_for_author, connector.list_comments):
