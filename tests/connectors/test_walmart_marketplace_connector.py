...
def _connector() -> tuple[WalmartMarketplaceToolSet, MockTransport]:
    transport = MockTransport()
    connector = WalmartMarketplaceToolSet(access_token="wm_secret", transport=transport)
...
def test_walmart_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "WALMART_list_items": "items",
        "WALMART_list_orders": "orders",
    }
...
def test_walmart_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_items, connector.list_orders):
