...
def _connector() -> tuple[EbayToolSet, MockTransport]:
    transport = MockTransport()
    connector = EbayToolSet(access_token="tok_secret", transport=transport)
...
def test_ebay_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "EBAY_search_items": "items",
        "EBAY_list_orders": "orders",
    }
...
def test_ebay_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.search_items, connector.list_orders):
