...
def _connector() -> tuple[TwilioToolSet, MockTransport]:
    transport = MockTransport()
    connector = TwilioToolSet(
        account_sid="AC_secret", auth_token="token_secret", transport=transport
    )
...
def test_twilio_list_tools_register_first_class_output_schemas() -> None:
...
def test_twilio_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_messages)
