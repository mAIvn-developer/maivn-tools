...
def _connector() -> tuple[LoopsToolSet, MockTransport]:
    transport = MockTransport()
    connector = LoopsToolSet(api_key="loops_secret", transport=transport)
...
def test_loops_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "LOOPS_list_mailing_lists": "lists",
    }
...
def test_loops_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_mailing_lists)
