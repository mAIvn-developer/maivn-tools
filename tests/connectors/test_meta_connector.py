...
def _connector() -> tuple[MetaToolSet, MockTransport]:
    transport = MockTransport()
    connector = MetaToolSet(access_token="secret", transport=transport)
...
def test_meta_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "META_list_page_posts": "posts",
        "META_list_post_comments": "comments",
    }
...
def test_meta_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_page_posts, connector.list_post_comments):
