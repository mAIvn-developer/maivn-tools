...
def _connector() -> tuple[OktaToolSet, MockTransport]:
    transport = MockTransport()
    connector = OktaToolSet(
        org_url="https://dev-123.okta.com",
        api_token="ssws_secret",
        transport=transport,
    )
...
def test_okta_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "OKTA_list_users": "users",
        "OKTA_list_groups": "groups",
        "OKTA_list_applications": "apps",
    }
...
def test_okta_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (
