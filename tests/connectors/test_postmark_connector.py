...
def _connector() -> tuple[PostmarkToolSet, MockTransport]:
    transport = MockTransport()
    connector = PostmarkToolSet(server_token="pm_secret", transport=transport)
...
def test_postmark_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "POSTMARK_list_templates": "templates",
        "POSTMARK_list_outbound_messages": "messages",
    }
...
def test_postmark_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_templates, connector.list_outbound_messages):
