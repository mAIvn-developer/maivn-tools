...
def _connector() -> tuple[SegmentToolSet, MockTransport]:
    transport = MockTransport()
    connector = SegmentToolSet(public_api_token="tok", transport=transport)
...
def test_segment_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "SEGMENT_list_sources": "sources",
        "SEGMENT_list_destinations": "destinations",
        "SEGMENT_list_warehouses": "warehouses",
    }
...
def test_segment_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_sources, connector.list_destinations, connector.list_warehouses):
