...
def _connector() -> tuple[YouTubeToolSet, MockTransport]:
    transport = MockTransport()
    connector = YouTubeToolSet(token="t", transport=transport)
...
def test_youtube_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "YOUTUBE_list_videos": "videos",
        "YOUTUBE_search": "videos",
        "YOUTUBE_list_comment_threads": "comments",
    }
...
def test_youtube_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (
