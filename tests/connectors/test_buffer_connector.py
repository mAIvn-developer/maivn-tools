...
def _connector() -> tuple[BufferToolSet, MockTransport]:
    transport = MockTransport()
    connector = BufferToolSet(api_key="t", transport=transport)
...
def test_buffer_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "BUFFER_list_pending_updates": "updates",
        "BUFFER_list_sent_updates": "updates",
    }
...
def test_buffer_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_pending_updates, connector.list_sent_updates):
