...
def _connector() -> tuple[DopplerToolSet, MockTransport]:
    transport = MockTransport()
    connector = DopplerToolSet(token="dp.st.secret", transport=transport)
...
def test_doppler_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "DOPPLER_list_projects": "projects",
        "DOPPLER_list_configs": "configs",
        "DOPPLER_list_secrets": "secrets",
    }
...
def test_doppler_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_projects, connector.list_configs, connector.list_secrets):
