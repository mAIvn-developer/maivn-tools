...
def _connector() -> tuple[JiraToolSet, MockTransport]:
...
def test_jira_list_tools_register_first_class_output_schemas() -> None:
...
def test_jira_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.search_issues, connector.list_projects):
