...
def _connector() -> tuple[ThreadsToolSet, MockTransport]:
    transport = MockTransport()
    connector = ThreadsToolSet(access_token="tok_secret", transport=transport)
...
def test_threads_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "THREADS_list_threads": "posts",
        "THREADS_list_replies": "replies",
    }
...
def test_threads_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_threads, connector.list_replies):
