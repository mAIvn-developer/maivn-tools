...
def _connector() -> tuple[JumpCloudToolSet, MockTransport]:
    transport = MockTransport()
    connector = JumpCloudToolSet(api_key="jc_secret", transport=transport)
...
def test_jumpcloud_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "JUMPCLOUD_list_users": "users",
        "JUMPCLOUD_list_user_groups": "groups",
        "JUMPCLOUD_list_systems": "systems",
        "JUMPCLOUD_list_applications": "apps",
    }
...
def test_jumpcloud_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (
        connector.list_users,
        connector.list_user_groups,
        connector.list_systems,
