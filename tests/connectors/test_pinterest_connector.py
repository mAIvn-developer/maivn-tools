...
def _connector() -> tuple[PinterestToolSet, MockTransport]:
    transport = MockTransport()
    connector = PinterestToolSet(access_token="pat_secret", transport=transport)
...
def test_pinterest_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "PINTEREST_list_boards": "boards",
        "PINTEREST_list_board_pins": "pins",
        "PINTEREST_list_pins": "pins",
    }
...
def test_pinterest_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_boards, connector.list_board_pins, connector.list_pins):
