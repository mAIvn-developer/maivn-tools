...
def _connector() -> tuple[InstagramToolSet, MockTransport]:
    transport = MockTransport()
    connector = InstagramToolSet(access_token="tok_secret", transport=transport)
...
def test_instagram_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "INSTAGRAM_list_media": "media",
        "INSTAGRAM_list_comments": "comments",
    }
...
def test_instagram_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_media, connector.list_comments):
