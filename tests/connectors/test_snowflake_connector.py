...
def _connector() -> tuple[SnowflakeToolSet, MockTransport]:
    transport = MockTransport()
    connector = SnowflakeToolSet(account="xy12345.us-east-1", token="tok", transport=transport)
...
def test_snowflake_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "SNOWFLAKE_list_databases": "databases",
        "SNOWFLAKE_list_schemas": "schemas",
        "SNOWFLAKE_list_tables": "tables",
        "SNOWFLAKE_list_views": "views",
        "SNOWFLAKE_list_warehouses": "warehouses",
    }
...
def test_snowflake_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_tables)
