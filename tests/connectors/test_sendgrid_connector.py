...
def _connector() -> tuple[SendGridToolSet, MockTransport]:
    transport = MockTransport()
    connector = SendGridToolSet(api_key="SG.xxx", transport=transport)
...
def test_sendgrid_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "SENDGRID_list_templates": "templates",
        "SENDGRID_list_lists": "lists",
    }
...
def test_sendgrid_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_templates, connector.list_lists):
