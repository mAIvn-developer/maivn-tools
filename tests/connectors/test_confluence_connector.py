...
def _connector() -> tuple[ConfluenceToolSet, MockTransport]:
...
def test_confluence_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "CONFLUENCE_list_content": "results",
        "CONFLUENCE_search": "results",
        "CONFLUENCE_search_content": "results",
    }
...
def test_confluence_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_content, connector.search, connector.search_content):
