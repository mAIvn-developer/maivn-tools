...
def _connector() -> tuple[KlaviyoToolSet, MockTransport]:
    transport = MockTransport()
    connector = KlaviyoToolSet(api_key="key_secret", transport=transport)
...
def test_klaviyo_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "KLAVIYO_list_profiles": "profiles",
        "KLAVIYO_list_lists": "lists",
        "KLAVIYO_list_campaigns": "campaigns",
        "KLAVIYO_list_flows": "flows",
    }
...
def test_klaviyo_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_profiles)
