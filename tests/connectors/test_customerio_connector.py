...
def _connector() -> tuple[CustomerIOToolSet, MockTransport]:
    transport = MockTransport()
    connector = CustomerIOToolSet(app_api_key="app_secret", transport=transport)
...
def test_customerio_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "CUSTOMERIO_list_segments": "segments",
        "CUSTOMERIO_list_broadcasts": "broadcasts",
        "CUSTOMERIO_list_campaigns": "campaigns",
    }
...
def test_customerio_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_segments)
