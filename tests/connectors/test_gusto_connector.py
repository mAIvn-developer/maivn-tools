...
def _connector() -> tuple[GustoToolSet, MockTransport]:
    transport = MockTransport()
    connector = GustoToolSet(access_token="t", transport=transport)
...
def test_gusto_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "GUSTO_list_companies": "companies",
        "GUSTO_list_employees": "employees",
    }
...
def test_gusto_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_companies, connector.list_employees):
