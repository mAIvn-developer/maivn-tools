...
def _connector() -> tuple[BoxToolSet, MockTransport]:
    transport = MockTransport()
    connector = BoxToolSet(token="box_secret", transport=transport)
...
def test_box_list_tools_register_first_class_output_schemas() -> None:
...
def test_box_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_folder_items, connector.search):
