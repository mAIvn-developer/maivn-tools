...
def _connector() -> tuple[BrevoToolSet, MockTransport]:
    transport = MockTransport()
    connector = BrevoToolSet(api_key="key_secret", transport=transport)
...
def test_brevo_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "BREVO_list_contacts": "contacts",
        "BREVO_list_lists": "lists",
        "BREVO_list_email_campaigns": "campaigns",
    }
...
def test_brevo_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (
        connector.list_contacts,
        connector.list_lists,
        connector.list_email_campaigns,
    ):
