...
def _connector() -> tuple[BlueskyToolSet, MockTransport]:
    transport = MockTransport()
    connector = BlueskyToolSet(access_jwt="jwt", did="did:plc:1", transport=transport)
...
def test_bluesky_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "BLUESKY_get_timeline": "posts",
        "BLUESKY_get_author_feed": "posts",
        "BLUESKY_search_posts": "posts",
        "BLUESKY_get_followers": "users",
        "BLUESKY_get_follows": "users",
    }
...
def test_bluesky_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.get_timeline)
