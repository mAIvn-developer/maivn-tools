...
def _connector() -> tuple[LinearToolSet, MockTransport]:
    transport = MockTransport()
    connector = LinearToolSet(api_key="lin_secret", transport=transport)
...
def test_linear_list_tools_register_first_class_output_schemas() -> None:
...
def test_linear_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_issues)
