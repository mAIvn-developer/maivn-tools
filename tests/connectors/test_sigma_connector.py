...
def _connector() -> tuple[SigmaToolSet, MockTransport]:
    transport = MockTransport()
    connector = SigmaToolSet(access_token="tok_secret", transport=transport)
...
def test_sigma_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "SIGMA_list_workbooks": "workbooks",
        "SIGMA_list_datasets": "datasets",
        "SIGMA_list_members": "members",
    }
...
def test_sigma_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_workbooks, connector.list_datasets, connector.list_members):
