...
def _connector() -> tuple[BigQueryToolSet, MockTransport]:
    transport = MockTransport()
    connector = BigQueryToolSet(token="t", project_id="my-proj", transport=transport)
...
def test_bigquery_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "BIGQUERY_list_datasets": "datasets",
        "BIGQUERY_list_tables": "tables",
    }
...
def test_bigquery_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_datasets, connector.list_tables):
