# pyright: strict
"""Tests for the Deel connector's first-class output schemas."""
...
def _connector() -> tuple[DeelToolSet, MockTransport]:
    transport = MockTransport()
    connector = DeelToolSet(api_token="t", transport=transport)
...
def test_deel_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "DEEL_list_people": "employees",
        "DEEL_list_contracts": "contracts",
    }
...
def test_deel_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_people, connector.list_contracts):
