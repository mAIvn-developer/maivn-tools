...
def _connector() -> tuple[PostHogToolSet, MockTransport]:
    transport = MockTransport()
    connector = PostHogToolSet(
        personal_api_key="phx_secret",
        project_api_key="phc_public",
        project_id=1,
        transport=transport,
    )
...
def test_posthog_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "POSTHOG_list_events": "events",
        "POSTHOG_list_insights": "insights",
        "POSTHOG_list_persons": "persons",
    }
...
def test_posthog_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_events)
