...
def _connector() -> tuple[MailchimpToolSet, MockTransport]:
...
def test_mailchimp_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "MAILCHIMP_list_lists": "lists",
        "MAILCHIMP_list_members": "subscribers",
        "MAILCHIMP_list_campaigns": "campaigns",
    }
...
def test_mailchimp_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (connector.list_lists, connector.list_members, connector.list_campaigns):
