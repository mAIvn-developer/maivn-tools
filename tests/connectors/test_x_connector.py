...
def _connector() -> tuple[XToolSet, MockTransport]:
    transport = MockTransport()
    connector = XToolSet(bearer_token="bt", transport=transport)
...
def test_x_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "X_search_recent_tweets": "tweets",
        "X_get_user_tweets": "tweets",
        "X_get_user_mentions": "tweets",
        "X_get_followers": "users",
        "X_get_following": "users",
    }
...
def test_x_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.search_recent_tweets)
