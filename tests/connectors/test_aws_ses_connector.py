...
def _connector() -> AmazonSESToolSet:
    return AmazonSESToolSet(region="us-east-1", transport=MockTransport())
...
def test_ses_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "SES_list_identities": "identities",
        "SES_list_templates": "templates",
    }
...
def test_ses_output_schemas_not_published_through_metadata() -> None:
    connector = _connector()
    for method in (connector.list_identities, connector.list_templates):
