...
def _connector() -> tuple[TelegramToolSet, MockTransport]:
    transport = MockTransport()
    connector = TelegramToolSet(bot_token="123456:ABC-secret", transport=transport)
...
def test_telegram_list_tools_register_first_class_output_schemas() -> None:
...
def test_telegram_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.get_updates)
