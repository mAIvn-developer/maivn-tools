...
def _connector() -> tuple[LeverToolSet, MockTransport]:
    transport = MockTransport()
    connector = LeverToolSet(api_key="lever_secret", transport=transport)
...
def test_lever_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "LEVER_list_opportunities": "candidates",
        "LEVER_list_postings": "jobs",
        "LEVER_list_users": "users",
    }
...
def test_lever_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    for method in (
