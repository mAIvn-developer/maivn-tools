...
def _connector() -> tuple[CensusToolSet, MockTransport]:
    transport = MockTransport()
    connector = CensusToolSet(secret_token="t", transport=transport)
...
def test_census_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "CENSUS_list_sources": "sources",
        "CENSUS_list_destinations": "destinations",
        "CENSUS_list_models": "models",
        "CENSUS_list_syncs": "syncs",
        "CENSUS_list_sync_runs": "runs",
    }
...
def test_census_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_sources)
