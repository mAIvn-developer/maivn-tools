...
def _connector() -> tuple[ResendToolSet, MockTransport]:
...
def test_resend_list_tools_register_first_class_output_schemas() -> None:
...
def test_resend_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_contacts)
