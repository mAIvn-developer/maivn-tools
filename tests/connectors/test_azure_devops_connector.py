...
def _connector() -> tuple[AzureDevOpsToolSet, MockTransport]:
    transport = MockTransport()
    connector = AzureDevOpsToolSet(organization="acme", pat="pat", transport=transport)
...
def test_azure_devops_list_tools_register_first_class_output_schemas() -> None:
    ...
    expected: dict[str, str] = {
        "ADO_list_projects": "projects",
        "ADO_list_repositories": "repositories",
        "ADO_list_pull_requests": "pull_requests",
        "ADO_list_branches": "branches",
        "ADO_list_pipelines": "pipelines",
        "ADO_list_builds": "builds",
    }
...
def test_azure_devops_output_schemas_not_published_through_metadata() -> None:
    connector, _ = _connector()
    opts = get_toolify_options(connector.list_projects)
